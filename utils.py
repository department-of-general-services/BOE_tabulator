...
def get_boe_minutes(
    ...
    start_page = requests.get(
        minutes_url
    )  # grab the 'starting page' that provides links to the minutes for a specific year
    ...
    year_tags = start_soup.find_all(
        "a", href=True, text=re.compile(r"^\d{4}$")
    ...
    for link in year_links:
        ...
        minutes_tags = soup.find_all(
            "a", text=date_regex
        )  # grab tags that link to the pdfs
        ...
        for tag in minutes_tags:
            ...
            try:  # most links will fit this pattern
                ...
                print(
                    f'Error "{e}" for minutes on: "{tag.string}", '
                ...
                for (
                    ...
                    typo_dict.items()
                ):  # this loop searches for matches among single-letter deletions
                    correct_month = False
                    if month_error in v:
                        correct_month = k
                        break
                ...
                if correct_month:  # if we found a match
                    ...
                    print(
                        f"Error: could not match month string "
                        f"{file_name_re.group(1)} in "
                        f"match {file_name_re.groups()}. "
                    )

            if os.path.exists(
                pdf_save_dir + file_name
            ):  # skip the download if we've already done it
                print(f"skipping: {file_name}")
            else:
                if tag.get("href").startswith(
                    "http"
                ):  # because there is literally ONE link in the entire list that is an absolute path
